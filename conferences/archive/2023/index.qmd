---
pagetitle: "2023 American Psychology-Law Society Conference" 
toc: false
css: index.css
image: "images/independence_hall.png"
description: | 
  The annual conference for the American Psychology-Law Society for 2023 will be held in Philadelphia from March 16-18.
includesource: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(aplsr)     # rowwise_table()
library(htmltools) # div(), tags$*, a(), span()
library(purrr)     # pmap()
```

```{=html}
    <body id="page-top" data-spy="scroll" data-target="side-menu">
      <nav class="side-menu">
        <ul>
          <li class="hidden active">
            <a class="page-scroll" href="#page-top" aria-hidden="true" tabindex="-1"></a>
          </li>
          <li>
            <a href="#welcome" class="page-scroll">
              <span class="menu-title">Welcome</span>
              <span class="dot"></span>
            </a>
          </li>
          <li>
          <a href="#hotel" class="page-scroll">
              <span class="menu-title">Hotel</span>
              <span class="dot"></span>
            </a>
          </li>
          <li>
            <a href="#schedule" class="page-scroll">
              <span class="menu-title">Schedule</span>
              <span class="dot"></span>
            </a>
          </li>
          <li>
            <a href="#sponsors" class="page-scroll">
              <span class="menu-title">Exhibitors</span>
              <span class="dot"></span>
            </a>
          </li>
        </ul>
      </nav>
```
::: column-screen
::: hero-banner

::: content-block
::: heading-guard
:::
# 2023 Annual Conference of the American Psychology-Law Society {.h2}

[Philadelphia, PA \| March 16-18, 2023]{.h4}

::: hero-buttons
[Register now](#register){#btn-guide .btn-action .btn-action .btn .btn-success role="button"} [Conference Program](schedule.html){.btn-action .btn .btn-secondary role="button"}
:::
:::

:::
:::

::: simple-info
::: {.content-block .col-v-center}
## 2023 AP-LS Conference has gone mobile!
:::
:::

::: grid
::: {.g-col-12 .g-col-md-6 .col-v-center}
> ### Get the Guide on your mobile device.
>
> tap "Download the app" to access the guide on your iOS or Android device.

![](images/guidebook_qrc_2023.png){fig-align="center" width="160" fig-alt="QR code to download the 2023 conference guidebook app"}

> Prefer the web version? Some content and networking tools may be available to you online.

[Download guidebook](https://guidebook.com/g/#/guides/apls2023/details){.btn-action-primary .btn-action .btn .btn-primary role="button"}
:::

::: {.g-col-12 .g-col-md-6 .col-v-center}
![](images/guidebook_phone.png){width="300" fig-alt="Phone showing the 2023 conference guidebook app"}
:::
:::

::: column-screen
```{r}
#| label: TOC
#| echo: false
#| warning: false
#| message: false

content = rowwise_table(~name, ~icon, ~link,
  "Hotel", "airplane", "#hotel",
  "Registration", "person-badge", "#register",  
  "Workshops", "person-workspace", "preconference.html",
  "Program", "calendar-event", "schedule.html",
  "Sponsors", "bag-check", "#sponsors"
)
div(class = "bg-light",
 div(class = "bg-breakout",
    div(class = "inner-container",
      div(class = "boxed-wrapper",
            div(class = "boxed-feature-grid",
          pmap(content, function(name, icon, link) {
            div(class = "boxed-feature-grid__item",
              div(class = "boxed-feature-grid__icon",
                a(href = link, class = "boxed-feature-grid__link", `aria-label` = name,
                  tags$i(class = sprintf("bi-%s boxed-feature-grid__i", icon))
                )),
    div(class = "boxed-feature-grid__title", name)
)
          })
        )
      )
    )
  )
)

```
:::

### Welcome to Philadelphia! {#welcome}

<figure class="text-end">

<blockquote class="blockquote">

<p class="mb-0">

We hope that you find the 2023 conference enriching, invigorating and collaborative. We are so excited to be in Philadelphia with all of you. Enjoy!

</p>
</blockquote>
<figcaption class="blockquote-footer">
2023 AP-LS Conference Co-Chairs <cite title="Andrea Avila & Miko M. Wilford">Andrea Avila & Miko M. Wilford</cite>
</figcaption>
</figure>

::: {.panel-tabset .box-shadow}
### {{< fa recycle >}} Mini Programs

#### Printed mini programs in lieu of full programs

As a step toward being greener and cutting down on costs as an organization, we decided to forego the printing of the full programs again this year. Instead, conference goers will receive printed mini programs. 

Conference goers can refer to the [schedule page](schedule.html) or full program PDF or the conference program mobile app via their digital devices.

### {{< fa wifi >}} Internet

#### Wireless internet limitations in conference rooms
::: {.callout-caution}
While those staying at the conference hotels will have complimentary internet access in their individual rooms, the actual conference rooms will not include wireless Internet access. 

Thus, we recommend downloading the full program PDF and the conference mobile app prior to arriving in Philadelphia.
:::

### {{< fa head-side-cough >}} COVID Precautions

To make the conference as safe as possible, we have tests available on demand to conference attendees.

You may contact `office@ap-ls.org` to arrange a contactless delivery of a COVID test.

::: callout-important
If you are experiencing any COVID symptoms, do not visit any conference spaces.
:::

### {{< fa person-running >}} Fun Run

![](images/fun-run.png){fig-align="center" fig-alt="2023 conference fun run"}

#### Explore Philadelphia with colleagues on a 5k Fun Run, sponsored by the AP-LS Student Committee!

::: callout-note
[**MEET IN LOBBY OF THE CONFERENCE HOTEL AT 7:00 AM**]{.h5}

REFRESHMENTS WILL BE PROVIDED AFTER THE RACE

[COST: \$25.00]{.badge .bg-warning}

***Funds raised are used to support student committee conference awards***

{{< downloadthis files/2023_AP-LS_Fun_Run_Waiver.pdf dname="2023_AP-LS_Fun_Run_Waiver.pdf" label="Liability Waiver" icon="download" type="primary" >}}
:::

### {{< fa graduation-cap >}}Continuing Education (CE)

![](images/concept.png){fig-align="center" fig-alt="Illustration about continuing education credits"}

Palo Alto University, Continuing & Professional Studies (CONCEPT) is pleased to offer CEs for the 2023 AP-LS conference. Numbered sessions in the schedule (p. 5 of the full program; see p. 20-22 for more detailed information) are expected to be eligible for CE credit; however, the certification process is still underway.

::: callout-tip
Please check the Guidebook app when planning session attendance to confirm which sessions are eligible for CE credit.

Our team will send an email to everyone who indicates they would like CEs on March 13th. All email communications will come from `caps@paloaltou.edu`Please make sure this email is whitelisted on your email provider.
:::

#### What to expect at the conference

-   To receive CEs for a conference session, you must attend the session in its entirety. There will be sign-in/sign-out sheets for each session located at the back of the room; please print your full name & email address clearly to document your attendance at the session. This must be done for each CE session you attend.

-   You must complete an evaluation for each CE-eligible session you attend. The evaluation links for each day of the conference will be sent via email prior to the start of the first session. Please do not access these links until after each of the live conference sessions.

-   You will be able to download & save a PDF certificate upon completion of the evaluation. A copy will also be emailed to the email used at registration within 1-week of the close of the conference.

-   If you need support from CONCEPT for CE/Certificate needs, please visit the CONCEPT booth (between Franklin Hall and the escalators) during the conference.

### {{< fa circle-info >}} For Presenters

#### Symposia, Paper, and Data Blitz Sessions

Symposia sessions are 80 minutes long and paper and data blitz sessions are 60 minutes long. All sessions will have an assigned chair who will be responsible for ensuring that approximately 10 minutes are reserved for questions. The remaining time should be divided evenly across all the scheduled presenters.

::: callout-caution
While Audio/Visual should be available in session rooms, note that Internet will not be. Thus, presenters need to ensure that their A/V content is stored on their computer and thumb drive so that it can be transferred to the session room computer and played locally.
:::

#### Posters

Poster Sizing: The poster boards for mounting a poster are 4 ft (height) x 8 ft (width). Presenters are free to print any size poster that will fit within this space. Pushpins will be provided.

Poster Board Numbering: Posters are numbered in the conference program so that the number for the poster indicates the correct board on which the poster should be hung. Poster presenters should refer to these numbers to identify the poster board to hang their poster.

### {{< fa chalkboard-user >}} Preconference Workshops

Pre-conference Workshops will take place on Wednesday March 15.

[Learn more](preconference.html){.btn-action .btn .btn-secondary .btn-lg role="button"}
:::

::: vspace2
:::

::: column-screen
::: {.simple-info-blue .alt-background-blue}
::: content-block-blue
#### The 2023 AP-LS Program Grid and Schedule is available to download and view online!

[Conference Program {{< fa arrow-right >}}](schedule.html){.btn-action .btn .btn-secondary .btn-lg role="button"}
:::
:::
:::

::: vspace2
:::

::: {simple-info}
## Hotel Information {#hotel}

::: {.panel-tabset .box-shadow}
### Philadelphia Marriott Downtown

![](images/hotel.jpeg){fig-align="center" fig-alt="Philadelphia Marriott Downtown, the 2023 conference hotel"}

#### The 2023 AP-LS Conference will be held at the [Philadelphia Marriott Downtown](https://www.marriott.com/en-us/hotels/phldt-philadelphia-marriott-downtown/overview/?scid=8e4624f4-e96a-4d68-b772-59df755b5c65&gclsrc=ds)

> 1201 Market Street </br> Philadelphia, PA, USA, 19107 <br> Guest entrance at 1200 Filbert Street

[{{< fa map-location-dot >}} Get Directions](https://www.google.com/maps?daddr=39.9526295,-75.16031){.btn-action .btn .btn-info .ml-auto role="button"}

### Conference Rates

Book a room through the AP-LS sponsored link for conference discount

\$209 USD per night\*

Rate not inclusive of final taxes/fees

[{{< fa bell-concierge >}} Book a room](https://book.passkey.com/gt/218524854?gtid=ebf7480702a2b34d863a79c09cb5f074){.btn-action .btn .btn-info role="button"}
:::
:::

::: vspace2
:::

::: {.column-screen .alt-background}
::: {.content-block}

## Registration {#register}

::: {.simple-info}
### Online registration is now closed. 
[You may still register in person at the conference.]{.h5}


::: {.callout-note collapse="true"}
### REGISTRATION RATES & INCLUSIONS

+-----------------------------------------+-----------------+---------------+
| Status                                  | Early Bird\     | Regular\      |
|                                         | (before Jan 31) | (on Feb 1)    |
+:========================================+:================+:==============+
| Full Member                             | \$245           | \$275         |
+-----------------------------------------+-----------------+---------------+
| Early Career Professional\              | \$180           | \$210         |
| Member                                  |                 |               |
+-----------------------------------------+-----------------+---------------+
| Student Member                          | \$65            | \$95          |
+-----------------------------------------+-----------------+---------------+
| Student Member *First-Author Presenter* | \$25            | \$25          |
+-----------------------------------------+-----------------+---------------+
| Student Non-member                      | \$115           | \$145         |
+-----------------------------------------+-----------------+---------------+
| Non-member                              | \$330           | \$400         |
+-----------------------------------------+-----------------+---------------+
:::

::: vspace
:::

:::
:::
:::

::: vspace
:::



::: {.column-screen}
::: {.content-block}

## Schedule Overview {#program}

::: {.simple-info}

::: {.panel-tabset .box-shadow}

### Preconference

#### Wednesday, March 15, 2023
[Preconference sessions](preconference.html) throughout the day

### Day 1 (Thurs)

#### Thursday, March 16, 2023
**All times are shown in EST**

+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Time** | **Event**                                                                                                                                                                  |
+==========+============================================================================================================================================================================+
| 10:00 AM | **Student Committee** Welcome Breakfast and Conference Orientation                                                                                                         |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 10:30 AM | **Student Committee** Professional Development Event: A Legal Workshop                                                                                                     |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 12:00 PM | **Opening Plenary:** Race Biases in the Legal System                                                                                                                       |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 2:15 PM  | **Teaching, Training and Careers (TTC) Committee:** Supporting Financial Literacy in Psychology: Income, Debt, & Satisfaction In Clinical & Nonclinical Training & Careers |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 2:15 PM  | **Corrections Committee:** Breaking Through: Navigating Research in Correctional Settings                                                                                  |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 2:15 PM  | **Student Committee:** The Missing Links: Bridging the Gaps Between Psychological Science and the Justice System                                                           |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 5:00 PM  | **Presidential Address:** Expanding our commitment to diversity                                                                                                            |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 2:15 PM  | **Teaching, Training and Careers (TTC) Committee:** Supporting Financial Literacy in Psychology: Income, Debt, & Satisfaction In Clinical & Nonclinical Training & Careers |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 5:00 PM  | Presidential Address                                                                                                                                                       |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 6:00 PM  | AP-LS Business Meeting (open to all!)                                                                                                                                      |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 6:30 PM  | Welcome Reception (open to all!)                                                                                                                                           |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 7:00 PM  | University of Denver Reception                                                                                                                                             |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 8:00 PM  | University of New Mexico Reception                                                                                                                                         |
+----------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------+

### Day 2 (Fri)

#### Friday, March 17, 2023
**All times are shown in EST**

+-----------+-------------------------------------------------------------------------------------------+
| **Time**  | **Event**                                                                                 |
+===========+===========================================================================================+
| 7:00 am   | **Student Committee** Sponsored 5k Fun Run                                                |
+-----------+-------------------------------------------------------------------------------------------+
| 10:45 am  | **Plenary: Alford Pleas:** Plea Convictions Without Guilt Admissions                      |
+-----------+-------------------------------------------------------------------------------------------+
| 1:30 pm   | **Practice Committee:** Death by a Thousand Cuts: Race, Mental Health, & Criminal Justice |
+-----------+-------------------------------------------------------------------------------------------+
| 1:30 pm   | **Legal Scholars Committee:** Judging Psychological Evidence On & Off the Bench           |
+-----------+-------------------------------------------------------------------------------------------+
| 3:00 pm   | **BRIDGE Committee:** The Fight for Gender Justice Post-Roe: Women's Law Project          |
+-----------+-------------------------------------------------------------------------------------------+
| 3:00 pm   | **Special Session:** Law & Science Dissertation Grant Program                             |
+-----------+-------------------------------------------------------------------------------------------+
| 5:20 pm   | **2023 Saleem Shah Award:** Lauren E. Kois, What's Your "So What?" Factor?                |
+-----------+-------------------------------------------------------------------------------------------+
| 6:30 pm   | Friday Poster Session                                                                     |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | **Early Career Professionals** Reception                                                  |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | **Student Committee** Reception                                                           |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | **BRIDGE Committee** Reception                                                            |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | University of Nebraska-Lincoln Reception                                                  |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | University of California, Irvine Reception                                                |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | Drexel/UMass/Fordham Reception                                                            |
+-----------+-------------------------------------------------------------------------------------------+
| 8:00 pm   | Sam Houston State University Reception                                                    |
+-----------+-------------------------------------------------------------------------------------------+

### Day 3 (Sat)

#### Saturday, March 18, 2023
**All times are shown in EST**

+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Time** | **Event**                                                                                                                                                                         |
+==========+===================================================================================================================================================================================+
| 9:15 am  | **Professional Development of Women Committee:** There is Never a Good Time: Considerations Surrounding Family Planning & Early Career Objectives                                 |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 10:45 am | **Presidential Plenary:** Is Behavioral Realism about Implicit Bias Changing Antidiscrimination Law?                                                                              |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 12:20 pm | **Special Session:** Development of an APA/AP-LS Collaboration on Amicus Briefs                                                                                                   |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 1:30 pm  | **Practice Committee:** Considering the Impact of Racial Trauma in Forensic Evaluations                                                                                           |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 3:00 pm  | **Practice Committee** Networking Event                                                                                                                                           |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 3:00 pm  | **2023 AP-LS Distinguished Contribution Award:** Patricia A. Griffin, A Community Psychologist in a Forensic World: Building Services, Systems & Partnerships to Make It All Work |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 4:30 pm  | **Fellows Committee** Meet-and-Greet                                                                                                                                              |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 6:30 pm  | Saturday Evening Poster Session                                                                                                                                                   |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| 8:00 pm  | John Jay College of Criminal Justice Reception                                                                                                                                    |
+----------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
:::

[Conference Program {{< fa arrow-right >}}](schedule.html){.btn-action-primary .btn-action .btn .btn-primary .btn-lg role="button"}
:::
:::

:::



::: vspace
:::


::: {.column-screen .alt-background}
::: {.content-block}

## Exhibitors & Sponsors {#sponsors}

::: {.simple-info}
::: {.callout-note collapse="true"}
### Exhibitor Pospectus
{{< downloadthis files/AP-LS_Prospectus_2023.pdf dname="AP-LS_Prospectus_2023" label="Download" icon="download" type="primary" >}}
:::

::: {.panel-tabset .box-shadow}
### 2023 Conference Sponsors

```{r}
#| echo: false
#| label: sponsors
#| warning: false
#| message: false
#| fig.alt: "Logos of the 2023 conference exhibitors and sponsors"

library(magick)
library(purrr)

# Read in all the images
pics <-
  fs::dir_ls("sponsors/") |>
  map(image_read) 

all_pics <-
  pics |>
  map(image_border, geometry = "10x10", color = "transparent") |>
  reduce(c) |>
  image_montage(
    tile = "2x2",
    geometry = geometry_area(width = 600),
    bg = "transparent"
  )
all_pics
```

### Become an AP-LS Sponsor

Signing up as a Business Partner of AP-LS has never been easier. Select your sponsorship, upload your logo, register your staff and make a payment all in one place.

[Click here to register!](https://apls.memberclicks.net/2023_sponsor_exhibitor_form)

### Take-One Brochure Table

\$200 (One time fee)

For the benefit of those who do not, or cannot, rent a booth exhibit, a Take-One Brochure Table for the distribution of free brochures and other material is available to offer pamphlets, brochures, leaflets, cards, magazines, etc. This will be for paid Take-One Exhibitors ONLY. Other materials will be removed. Unused materials will not be returned.

#### Branding opportunities

All targeted sponsorship opportunities come with acknowledgment at the conference.

\*Cost production not included in sponsorship

-   Tote Bags - \$2,000
-   Lanyards - \$2,000
-   Wireless Mobile Device Charger - \$3,500
-   USB Drive - \$2,500
-   Key Card - \$9,500
-   Keycard Holder Multi-page Pocket Schedule - \$10,500
-   Bag Insert - \$250
-   Scholarships - \$3,000
-   Mobile App Push Notifications - \$500

### Sponsorship opportunities

All targeted sponsorship opportunities support the individual items or events selected.

Each opportunity is exclusive, so your organization will be featured prominently at each event.

-   **Conference Wi-Fi Sponsor -** \$8,000

-   **Welcome Reception Sponsor -** \$8,000

-   **Thursday Breaks (AM/PM) -** \$3,500

-   **Lunch Product Theatre (2 available - Fri or Sat) -** \$15,000 (Lunch not included in pricing

-   **Poster Session Reception (2 available - Fri or Sat) -** \$10,000

-   **Friday Breaks (AM/PM) -** \$3,500

-   **Saturday Breaks (AM/PM) -** \$2,000
:::

:::
:::

:::
