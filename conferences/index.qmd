...
```{r cfp-details-button}
#| echo: false
#| results: asis

if (cfp_status == "open" && nzchar(info$submission_portal_url)) {
  cat(sprintf(
    '[Submit a Proposal {{< fa arrow-right aria-hidden="true" >}}](%s){.btn-action .btn .btn-action-primary .btn-lg role="button"}',
    info$submission_portal_url
  ))
} else {
  cat('[Full CFP Details {{< fa arrow-right aria-hidden="true" >}}](cfp/index.qmd){.btn-action .btn .btn-secondary .btn-lg role="button"}')
}
```
