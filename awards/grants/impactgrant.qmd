...
date: last-modified
description: Small/seed grants ($5,000-$10,000) and large grants ($15,000-$50,000) for interdisciplinary research that addresses understudied topics in psychology and law.
categories: 
  - Research
submission-info: 
  sponsor: AP-LS Research Committee
  deadline: 
  - "October 16, `r if (Sys.Date() > lubridate::mdy(paste('October 15', lubridate::year(Sys.Date())))) lubridate::year(Sys.Date()) + 1 else lubridate::year(Sys.Date())` (Pre-proposal)"
  - "Mid-December, `r if (Sys.Date() > lubridate::mdy(paste('Mid-December', lubridate::year(Sys.Date())))) lubridate::year(Sys.Date()) + 1 else lubridate::year(Sys.Date())` (Full-proposal)"
links:
- icon: envelope-at
  name: Contact
  url: "mailto:research@ap-ls.org"
- icon: send
  name: Apply here
...
-   Small/Seed proposals (\$5,000-\$10,000 for a 12-month research period)
-   Large proposals (\$15,000-\$50,000 over a 12-18 month research period)


::: callout-tip
Funding should primarily support research activities and data collection efforts. Support for research assistants and travel may be requested, but should not constitute the majority of the requested funds. One-time conference travel is limited to the PI and/or co-PI only.
...
Applicants should carefully consider anticipated project costs and associated budget expenditures. In the event of proposal funding, budget reallocation requests will be considered only in exceptional circumstances. All budget reallocation requests must be aligned with the original scope of work and be necessary to fulfill the project objectives.

**New for 2026-2027 applications:** funds cannot be used to support the salary of the PI or co-PI. Funds must be used for expenses associated with research activities, data collection, and dissemination. 

Please email one or both of the co-chairs of the Research Committee, Mel Baker or Evan Lowder, with any questions: {{< var email.research >}}

:::

## Eligibility
Applicants may submit only one small and one large proposal per funding cycle. Each proposal must describe a unique project; you may not submit the same project as both a large and small proposal in the same cycle.
...
Interested researchers should submit a 1,000-word prospectus to the committee by {{< meta submission-info.deadline.1 >}} at 11:59 p.m. PST. All materials are submitted through the Div. 41 (AP-LS) awards page.
...
Based on review of the pre-proposals, investigators will be notified in November if they are invited to submit a full proposal application. Full proposal applications will be due by Mid-December, `r format(Sys.Date(), '%Y')`.
...
For investigators invited to submit full proposal applications, full proposal applications are due in Mid-December, `r format(Sys.Date(), '%Y')`.
...
# Data: awards/data/reid.csv (refresh via awards/_update-awards.R)
data <- read.csv("../data/reid.csv") |>
  mutate(grant = paste(trimws(award_type), "grant"))

awards_table(data, name = "name", detail = "institution",
